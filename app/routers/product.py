...
async def add_product(product: Product):
    product_logger = get_logger("product_logger")
    success = random.choice([True, False])
    if not success:
        failure_cause = random.choice(["Product with same name already exists.", "Server Error", "Unauthorized"])
        product_logger.error(f"Create Product - Failed to create product for user ID {product.user_id}: {failure_cause}")
    ...
    product_logger.info(f"Create Product - Successfully created product {product_id} by user {product.user_id}")
    return {
...
async def update_product(product_id: int, product: Product):
    ...
    product_logger.info(f"Update Product - Successfully updated product {product_id} by user {product.user_id}")
    return {
...
async def get_product(product_id: int):
    ...
    product_logger.info(f"Get Product - Successfully retrieved product {product_id}")
    return {
        "product_id": product_id,
...
async def delete_product(product_id: int, product: Product):
    ...
    product_logger.info(f"Delete Product - Successfully deleted product {product_id} by user ID {product.user_id}")
    return {
